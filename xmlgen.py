...
def select_playlist2(id):
	payload = {'id':id}
	r = requests.get('http://localhost:5400/playlist/select', params=payload)
	json = r.json()
	return json
...
def get_xml(id):
	...
	songsInPlaylist = requests.get('http://localhost:5000/playlist/display', params=payload)
	songsInPlaylistJSON = songsInPlaylist.json()
	#songsInPlaylistDict = json.loads(songsInPlaylistJSON)

	playlistInfo = requests.get('http://localhost:5000/playlist/select', params=payload)
	#playlistInfoJSON = playlistInfo.json() <--- Unknown error cause
	#playlistInfoJSON = json.loads(playlistInfo.text)

	allSongs = requests.get('http://localhost:5100/api/resources/tracks/all')
	...
	for myArray in songsInPlaylistJSON:
		...
		for myArray2 in allSongsJSON:
			if str(myArray['track_id']) == str(myArray2['id']):
				...
				outputStringXML += "\t\t\t<info> " + str(myArray2['url']) + "</info>\r\r\n"
